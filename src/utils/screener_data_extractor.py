...
async def fetch_and_save_screener_data(screener_url: str):
    """
    Fetch data from Screener.in and save as CSV files.
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logger.info(f"📁 Saving CSV files to: {base_path}\n")
            
            # Save CSV files
            for file_info in csv_files:
                if file_info['data']:
                    save_csv(base_path / file_info['name'], file_info['data'])
                    logger.info(f"✅ Created: {file_info['name']} ({len(file_info['data'])} records)")
...
def regen_all_csvs():
    ...
    for stock_dir in sorted(base.iterdir()):
        ...
        try:
            ...
            logger.info(f"Regenerated CSVs for {stock_dir.name}")
        except Exception as e:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("    Re-generates P&L / quarterly / ratios CSVs for all stocks from saved page text.")
